...
export default function Builder({ selected, heroes, emphasis, clientName, activeOils, toggleOil, setHeroes, setEmphasis, setClientName, onBuild, onBack }) {
  const { t } = useLang()
  const totalSel = TIERS.reduce((s,ti)=>s+selected[ti].length,0)
  return (
    <div>
      {onBack && <button onClick={onBack} style={{ background:'none', border:'none', color:C.gold, fontSize:11, letterSpacing:'0.1em', textTransform:'uppercase', padding:0, cursor:'pointer', marginBottom:20 }}>{t.back}</button>}
      {TIERS.map(tier => {
          ...
          <div key={tier} style={{ marginBottom:22 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
              <span style={{ fontFamily:fH, fontSize:19, fontStyle:'italic' }}>
                {{top:t.note_top,middle:t.note_mid,base:t.note_base}[tier]} Note
              </span>
              ...
              {oils.map(oil => {
                ...
                return (
                  <button key={oil.id} onClick={()=>!isDis&&toggleOil(tier,oil)}
                    style={{ padding:'6px 12px', borderRadius:24, fontSize:12.5, cursor:isDis?'not-allowed':'pointer', fontWeight:oil.maxDrops!==undefined?500:300, transition:'all 0.1s', background:isSel?(isHero?C.gold:C.dark):'#FFF', color:isSel?'white':isDis?'#CCC':oil.maxDrops!==undefined?C.red:C.dark, border:`1px solid ${isSel?(isHero?C.gold:C.dark):isDis?'#EEE':oil.maxDrops!==undefined?C.red+'55':C.border}`, opacity:isDis?0.35:1 }}>
                    {oil.name}
                    {oil.maxDrops!==undefined&&<span style={{ fontSize:8, marginLeft:4, opacity:0.65 }}>MAX</span>}
                  </button>
                )
              })}
            </div>
      ...
      <button onClick={onBuild} disabled={totalSel===0}
        style={{ width:'100%', padding:15, border:'none', borderRadius:4, cursor:totalSel===0?'not-allowed':'pointer', fontSize:12, letterSpacing:'0.15em', textTransform:'uppercase', background:totalSel===0?C.border:C.dark, color:totalSel===0?C.mid:'white', transition:'all 0.2s' }}>
        {t.builder_btn}
      </button>
