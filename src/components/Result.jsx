...
export default function Result({ formula, clientName, emphasis, sampleNum, onEdit, onNew, onAdjust, onMix, onMake2, onFinalize }) {
  const { t } = useLang()
  const [showEdit, setShowEdit] = useState(false)
  const [editDrops, setEditDrops] = useState(() => {
    const m = {}
    formula.oils.forEach(o => { m[o.id] = o.drops })
    return m
  })
  ...
  const DROP = 0.02
  const saveEdit = () => {
    const updated = formula.oils.map(o => ({
      ...o,
      drops: parseInt(editDrops[o.id]) || 0,
      g: parseFloat(((parseInt(editDrops[o.id])||0)*DROP).toFixed(3)),
      pct: Math.round(((parseInt(editDrops[o.id])||0)/TOTAL_DROPS)*100),
    }))
    onAdjust(updated)
    setShowEdit(false)
  }

  const SCALES = [{ label: t.result_scale_30, factor: 15 }, { label: t.result_scale_50, factor: 22.5 }]
  ...
  return (
    <div className="screen" style={{ maxWidth: 600, margin: '0 auto', padding: '36px 20px 100px', color: C.dark, minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <button onClick={onEdit} style={{ background: 'none', border: 'none', color: C.gold, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', padding: 0, cursor: 'pointer' }}>{t.result_edit}</button>
        <div style={{ fontSize: 11, background: C.goldLight, color: C.gold, borderRadius: 20, padding: '4px 12px' }}>
          {t.result_sample} {sampleNum}
        </div>
      </div>

      <div style={{ marginBottom: 24 }}>
        <div style={{ fontFamily: fH, fontSize: 28, fontWeight: 300 }}>{clientName}</div>
        <div style={{ fontSize: 11, color: C.mid, marginTop: 2 }}>
          {new Date().toLocaleDateString('en-NZ',{day:'numeric',month:'long',year:'numeric'})} · {empLabel}
        </div>
      </div>
      ...
      {scaleIdx !== null && (
        <div className="fade" style={{ background: '#FFF', border: `1px solid ${C.border}`, borderRadius: 4, padding: '14px 16px', marginBottom: 16 }}>
          <div style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: C.gold, marginBottom: 10 }}>{SCALES[scaleIdx].label}</div>
      ...
      <button onClick={() => setShowEdit(v=>!v)}
        style={{ width: '100%', padding: '10px', background: '#FFF', color: C.mid, border: `1px solid ${C.border}`, borderRadius: 6, fontSize: 12, cursor: 'pointer', marginBottom: 14, letterSpacing: '0.1em' }}>
        ✏ {t.result_edit_btn}
      </button>
      ...
      {showEdit && (
        <div className="fade" style={{ background: '#FFF', border: `1px solid ${C.border}`, borderRadius: 8, padding: '16px', marginBottom: 16 }}>
          ...
          {formula.oils.map(oil => (
            <div key={oil.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <button onClick={() => setEditDrops(p=>({...p,[oil.id]:Math.max(0,(parseInt(p[oil.id])||0)-1)}))
                  } style={{ width: 28, height: 28, border: `1px solid ${C.border}`, borderRadius: 4, background: '#FFF', fontSize: 16, cursor: 'pointer' }}>−</button>
                <span style={{ fontFamily: fH, fontSize: 18, minWidth: 30, textAlign: 'center' }}>{editDrops[oil.id]||0}</span>
                <button onClick={() => setEditDrops(p=>({...p,[oil.id]:(parseInt(p[oil.id])||0)+1}))}
                  style={{ width: 28, height: 28, border: `1px solid ${C.border}`, borderRadius: 4, background: '#FFF', fontSize: 16, cursor: 'pointer' }}>+</button>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderTop: `1px solid ${C.border}`, marginBottom: 12 }}>
            <span style={{ fontSize: 12, color: C.mid }}>Total</span>
            <span style={{ fontFamily: fH, fontSize: 18, color: editValid ? C.dark : C.red }}>{editTotal} / 100 drops</span>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <button onClick={saveEdit} disabled={!editValid}
              style={{ padding: 12, background: editValid?C.dark:C.border, color: editValid?'white':C.mid, border: 'none', borderRadius: 6, fontSize: 12, cursor: editValid?'pointer':'not-allowed', letterSpacing: '0.1em' }}>
              {t.result_adj_save}
            </button>
            <button onClick={() => setShowEdit(false)}
              style={{ padding: 12, background: '#FFF', color: C.dark, border: `1px solid ${C.border}`, borderRadius: 6, fontSize: 12, cursor: 'pointer' }}>
              {t.result_adj_cancel}
            </button>
          </div>
        </div>
      ...
      <button onClick={onMix}
        style={{ width: '100%', padding: 15, background: C.dark, color: 'white', border: 'none', borderRadius: 8, fontSize: 12, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: 'pointer', marginBottom: 10 }}>
        {t.result_save} →
      </button>
    </div>
  )
