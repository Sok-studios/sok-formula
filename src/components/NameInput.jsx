...
export default function NameInput({ onContinue }) {
  ...
  return (
    <div className="screen" style={{ position: 'fixed', inset: 0, background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
      ...
      <div style={{ fontFamily: fH, fontSize: 34, fontWeight: 300, lineHeight: 1.1, marginBottom: 12, textAlign: 'center' }}>{t.name_title}</div>
      ...
      <button onClick={() => valid && onContinue(name.trim(), email.trim())} disabled={!valid}
        style={{ width: '100%', maxWidth: 320, padding: 15, background: valid?C.dark:C.border, color: valid?'white':C.mid, border: 'none', borderRadius: 6, fontSize: 12, letterSpacing: '0.15em', textTransform: 'uppercase', cursor: valid?'pointer':'not-allowed', transition: 'all 0.2s' }}>
        {t.start_btn}
      </button>
    </div>
  )
