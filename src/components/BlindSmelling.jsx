...
export default function BlindSmelling({ vialOrder, onDone, onBack }) {
  ...
  const handleDone = () => {
    const families = [...selected].sort((a,b)=>a-b).map(n=>vialOrder[n-1]).filter(Boolean)
    onDone(families)
  }
  return (
    <div className="screen" style={{ maxWidth:600, margin:'0 auto', padding:'36px 20px 100px', color:C.dark, minHeight:'100vh' }}>
      ...
      <div style={{ marginBottom:32 }}>
        <div style={{ fontFamily:fH, fontSize:10, letterSpacing:'0.35em', color:C.gold, textTransform:'uppercase', marginBottom:8 }}>Sok Studios · {t.blind_step}</div>
        <div style={{ fontFamily:fH, fontSize:34, fontWeight:300, lineHeight:1.1, marginBottom:12 }}>{t.blind_title}</div>
        ...
        {Array.from({length:vialOrder.length},(_,i)=>i+1).map(num => {
          ...
          return (
            <button key={num} onClick={()=>toggle(num)}
              ...
              <div style={{ fontFamily:fH, fontSize:28, fontWeight:300, color:sel?C.gold:C.mid }}>{num}</div>
              {sel && <div style={{ fontSize:14, color:C.gold }}>✓</div>}
            </button>
          )
        })}
      </div>
      {selected.size>0 && <div className="fade" style={{ fontSize:12, color:C.gold, textAlign:'center', marginBottom:16 }}>{selected.size} {t.blind_btn.split('→')[0]}</div>}
      <button onClick={handleDone} disabled={selected.size===0}
        style={{ width:'100%', padding:16, border:'none', borderRadius:8, background:selected.size>0?C.dark:C.border, color:selected.size>0?'white':C.mid, fontSize:13, letterSpacing:'0.12em', textTransform:'uppercase', cursor:selected.size>0?'pointer':'not-allowed', transition:'all 0.2s' }}>
        {t.blind_btn}
      </button>
