  ...
  return(
    <div className="screen" style={{maxWidth:600,margin:'0 auto',padding:'36px 20px 100px',color:C.dark,minHeight:'100vh'}}>
      ...
      <div style={{marginBottom:32}}>
        <div style={{fontFamily:fHead,fontSize:10,letterSpacing:'0.35em',color:C.gold,textTransform:'uppercase',marginBottom:8}}>Sok Studios · {t.howto_step}</div>
        <div style={{fontFamily:fHead,fontSize:34,fontWeight:300,lineHeight:1.1,marginBottom:12}}>{t.howto_title}</div>
          ...
          <div key={note.tier} style={{display:'flex',gap:16,marginBottom:16}}>
            ...
            <div style={{background:'#FFF',border:`1px solid ${C.border}`,borderRadius:8,padding:'14px 16px',flex:1}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                <span style={{fontFamily:fHead,fontSize:18,fontWeight:400}}>{note.tier}</span>
      ...
      <div style={{background:C.goldLight+'40',border:`1px solid ${C.goldLight}`,borderRadius:8,padding:'18px 20px',marginBottom:32}}>
        <div style={{fontFamily:fHead,fontSize:15,fontStyle:'italic',color:C.dark,lineHeight:1.7,textAlign:'center'}}>{t.howto_quote}</div>
      </div>
      <button onClick={onContinue} style={{width:'100%',padding:16,background:C.dark,color:'white',border:'none',borderRadius:8,fontSize:13,letterSpacing:'0.12em',textTransform:'uppercase',cursor:'pointer',fontFamily:fB}}>{t.howto_btn}</button>
    </div>
  )
}
