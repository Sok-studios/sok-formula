...
export default function PasswordGate({correctPassword,onSuccess}){
  ...
  const try_=()=>{if(input.trim().toLowerCase()===correctPassword.trim().toLowerCase())onSuccess();else{setError(true);setInput('')}}
  return(
    <div className="screen" style={{position:'fixed',inset:0,background:C.bg,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:32}}>
      ...
      <div style={{fontFamily:fHead,fontSize:30,fontWeight:300,marginBottom:10}}>{t.pwd_title}</div>
      ...
      <button onClick={try_} style={{marginTop:16,width:'100%',maxWidth:300,padding:15,background:C.dark,color:'white',border:'none',borderRadius:6,fontSize:12,letterSpacing:'0.15em',textTransform:'uppercase',cursor:'pointer',fontFamily:fB}}>{t.pwd_btn}</button>
    </div>
  )
}
