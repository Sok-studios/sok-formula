...
export async function saveFormula(data) {
  const { error } = await supabase.from('formulas').insert([{
    client_name: data.clientName||null, emphasis: data.emphasis, oils: data.oils,
    warnings: data.warnings, total_g: data.oils.reduce((s,o)=>s+o.g,0),
    total_drops: data.oils.reduce((s,o)=>s+o.drops,0),
    perfume_name: data.perfumeName||null, client_email: data.clientEmail||null,
  }])
  return error
}
...
export async function setSessionOils(ids) {
  const { error } = await supabase.from('session_config').upsert([{id:1,active_ids:ids,updated_at:new Date().toISOString()}])
  return error
}
export async function setSessionPassword(password) {
  const { error } = await supabase.from('session_config').upsert([{id:1,session_password:password||null,updated_at:new Date().toISOString()}])
  return error
}
export async function setVialOrder(order) {
  const { error } = await supabase.from('session_config').upsert([{id:1,vial_order:order,updated_at:new Date().toISOString()}])
  return error
}
export async function saveBlindResult(clientName, selectedFamilies) {
  const { error } = await supabase.from('blind_results').insert([{client_name:clientName,selected_families:selectedFamilies}])
  return error
}
...
export async function upsertOilOverride(oilId, maxDrops) {
  ...
  if (maxDrops!==null) {
    const { error } = await supabase.from('oil_config').insert([{type:'override',oil_id:oilId,tier:'',name:'',max_drops:maxDrops,active:true}])
    return error
  }
  return null
}
export async function addCustomOil(tier, name, maxDrops) {
  ...
  const { error } = await supabase.from('oil_config').insert([{type:'custom',oil_id:id,tier,name,max_drops:maxDrops||null,active:true}])
  return error
}
