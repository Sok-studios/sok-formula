    ...
    <>
      ...
      {screen==='builder'&&(
        <div style={{maxWidth:600,margin:'0 auto',padding:'36px 20px 80px',color:C.dark,minHeight:'100vh'}}>
          ...
          <Builder selected={selected} heroes={heroes} emphasis={emphasis} clientName={clientName} activeOils={activeOils} toggleOil={toggleOil} setHeroes={setHeroes} setEmphasis={setEmphasis} onBuild={doBuild} onBack={()=>setScreen('smresult')}/>
        </div>
      )}
      ...
      {screen==='postmix1'&&(
        <PostMixScreen lang={lang} t={t} sampleNum={1}
...
function PostMixScreen({lang,t,sampleNum,onMake2,onFinalize}){
  ...
  return(
    <div className="screen-up" style={{maxWidth:600,margin:'0 auto',padding:'36px 20px 100px',color:C.dark,minHeight:'100vh'}}>
      <div style={{marginBottom:32}}>
        ...
        <div style={{fontFamily:fHead,fontSize:30,fontWeight:300,lineHeight:1.1,marginBottom:8}}>{t.postmix_title}</div>
        <div style={{fontSize:13,color:C.mid,lineHeight:1.7}}>{t.postmix_sub}</div>
      </div>
      <textarea value={impression} onChange={e=>setImpression(e.target.value)} placeholder={t.postmix_imp_ph} rows={4}
        style={{width:'100%',padding:'14px',border:`1px solid ${C.border}`,borderRadius:8,fontSize:14,outline:'none',background:'#FFF',color:C.dark,resize:'none',lineHeight:1.6,marginBottom:24,fontFamily:fB}}/>
